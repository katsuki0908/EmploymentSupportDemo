...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    if (req.method === 'GET') {
        try {
            ...
            logger.info({ message: 'キャリアパスを取得できませんでした', error: error });
    ...
    //編集
        ...
        try {
            ...
            logger.info({ message: 'キャリアパスを更新しました', updatedData: result });
            res.status(200).json({ message: "キャリアパスを更新しました。", updatedData: result });

        }
        catch (error) {
            if (error instanceof Prisma.PrismaClientKnownRequestError) {
                logger.error({ message: '構文エラーでキャリアパスの更新に失敗しました', error: error });
                res.status(400).json({ error: "リクエストが無効です。" });
            } else {
                logger.error({ message: '予期せぬエラーでキャリアパスの更新に失敗しました', error: error });
    ...
    //追加
        ...
        try {
            ...
            logger.info({ message: 'キャリアパスを追加しました', updatedData: result });
            res.status(201).json(result);
        }
        catch (error) {
            if (error instanceof Prisma.PrismaClientKnownRequestError) {
                logger.error({ message: '構文エラーでキャリアパスの追加に失敗しました', error: error });
                res.status(400).json({ error: "リクエストが無効です。" });
            } else {
                logger.error({ message: '予期せぬエラーでキャリアパスの追加に失敗しました', error: error });
