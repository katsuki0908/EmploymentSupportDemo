...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    if (req.method === 'GET') {
        ...
        try {
            ...
            logger.info({ message: 'キャリアデータを取得できませんでした', error: error });
            if (error instanceof Prisma.PrismaClientKnownRequestError) {
                // Prismaが特定のエラーを検知した場合
                res.status(400).json({ error: "リクエストが無効です。" });
            } else {
                // その他のエラーの場合
                res.status(500).json({ error: "キャリアの取得に失敗しました。予期せぬエラーが発生しました。" });
            }
        }
    }
    ...
    //キャリアデータの編集
        ...
        const { career_action_id, student_id, action_date, notes, career_path_id, action_id } = req.body;
        try {
            ...
            res.status(200).json({ message: "キャリアを更新しました。", updatedData: result });
        }
        catch (error) {
            console.log("更新失敗", error);
            if (error instanceof Prisma.PrismaClientKnownRequestError) {
                logger.error({ message: '構文エラーでキャリアの更新に失敗しました', error: error });
                res.status(400).json({ error: "リクエストが無効です。" });
            } else {
                logger.error({ message: '予期せぬエラーでキャリアの更新に失敗しました', error: error });
    ...
    //キャリアデータの追加
        ...
        try {
            const { student_id, action_date, notes, career_path_id, action_id } = req.body;
            ...
            if (error instanceof Prisma.PrismaClientKnownRequestError) {
                logger.error({ message: '構文エラーでキャリア追加に失敗しました', error: error });
                res.status(400).json({ error: "リクエストが無効です。" });
            } else {
                logger.error({ message: '予期せぬエラーでキャリアの追加に失敗しました', error: error });
    ...
    //キャリアデータの削除
        ...
        try {
            ...
            if (error instanceof Prisma.PrismaClientKnownRequestError) {
                logger.error({ message: '構文エラーでキャリアの削除に失敗しました', error: error });
                res.status(400).json({ error: "リクエストが無効です。" });
            } else {
                logger.error({ message: '予期せぬエラーでキャリアの削除に失敗しました', error: error });
