...
export default async function handler(
  ...
  //既に会社名が追加されているその他のキャリアアクションを修正
    ...
    try {
      ...
      if (error instanceof Prisma.PrismaClientKnownRequestError) {
        // logger.error({
        //   message: "構文エラーで既存のキャリアパスへの更新に失敗しました",
        //   error: error,
        // });
        res.status(400).json({ error: "リクエストが無効です。" });
      } else {
  ...
  //会社名の追加とキャリアアクションの修正
    ...
    try {
      ...
      // logger.info({
      //   message: "新たなキャリアパスを追加しキャリアアクションを修正しました",
      //   updatedData: result,
      // });
      ...
      if (error instanceof Prisma.PrismaClientKnownRequestError) {
        // logger.error({
        //   message: "構文エラーでその他の情報の追加に失敗しました",
        //   error: error,
        // });
        res.status(400).json({ error: "リクエストが無効です。" });
      } else {
